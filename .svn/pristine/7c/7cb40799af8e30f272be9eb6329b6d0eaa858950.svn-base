...
public class LoginController {
    ...
    public ApiResult login(@RequestParam("loginName") String loginName, HttpSession session, @RequestParam("password") String password) throws Exception {
        ...
        claims.put(UserInfoKey.CURRENT_ORGNO,login.getOrgNo());
        claims.put(UserInfoKey.IS_SYSTEM,login.getIsSystem());
        //claims.put(RedisKeyConstant.USER_LOGIN+userId,login);
        String jwt = JWTUtil.createJWT(uuid, String.valueOf(userId),ttl,claims);

        // 5分钟内不用重复验证
        String key = MD5Util.encodeMD5Hex(jwt);
        session.setAttribute("md5Token",key);
        redisTemplate.opsForHash().putAll(key,claims);
        redisTemplate.opsForValue().set(RedisKeyConstant.USER_LOGIN+userId,login);
    ...
    public ApiResult logout(HttpServletRequest request) throws Exception {
       ...
       /* String md5Token = (String)request.getAttribute("md5Token");
        ...
        ApiResult result = new ApiResult();
        String md5Token = (String)request.getAttribute("md5Token");
        boolean delete = redisTemplate.delete(md5Token);
        log.info("退出登录，删除key{}",delete);
        boolean aBoolean = redisTemplate.hasKey(md5Token);
        log.info("是否还存在key{}",aBoolean);
        return result;
    }


    @SystemControllerLog(description = "验证密码")
    @ApiOperation(value="验证密码", notes="验证密码")
    @ApiImplicitParam(name = "password", value = "密码", required = true)
    @GetMapping(value = "/validate")
    public ApiResult checkPassword(HttpServletRequest request, @RequestParam("password") String password) throws Exception {
        String md5Token = (String)request.getAttribute("md5Token");
        Map entries = redisTemplate.opsForHash().entries(md5Token);
        ...
        // 查询用户信息
        userService.checkPassword(Integer.parseInt(userId),password);
        return result;
    }
