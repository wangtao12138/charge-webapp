...
import cn.com.cdboost.charge.webapi.dto.info.ICCardDetailResp;
...
public class ChargerICController {
    ...
    @ApiImplicitParam(name = "ICCardQueryParam", value = "ic卡列表查询对象", required = true)
    @GetMapping
    public ApiResult<PageData<ICCardResp>> queryList(HttpServletRequest request, @ModelAttribute ICCardQueryParam param){
        ...
        //查询数据库
        PageData<ICCardResp> cards = icCardWrapper.queryList(param, Integer.valueOf(userId));
    ...
    public void downloadList(HttpServletRequest request, HttpServletResponse response, ICCardQueryParam param){
        ...
        String userId = String.valueOf(entries.get(UserInfoKey.USER_GUID));
        //查询数据库
        PageData<ICCardResp> cards = icCardWrapper.queryList(param,Integer.parseInt(userId));
        try {
    ...
    @SystemControllerLog(description = "查询IC卡详情")
    @ApiOperation(value="查询IC卡详情", notes="查询IC卡详情")
    @ApiImplicitParam(name = "cardId", value = "卡号", required = true, dataType ="String")
    @GetMapping(value = "/detail/{cardId}")
    public ApiResult<ICCardDetailResp> queryCardDetail(@PathVariable String cardId){
        ApiResult result = new ApiResult();
        ICCardDetailResp icCardDetailResp = icCardWrapper.queryCardCustomerDetail(cardId);
        result.setData(icCardDetailResp);
        return result;
    }
    ...
    @ApiImplicitParam(name = "cardIds", value = "ic卡卡号集合", required = true)
    @DeleteMapping
    ...
    @ApiImplicitParam(name = "ICCardQueryParam", value = "ic卡下发列表查询参数", required = true)
    @GetMapping(value = "/cardSendList")
    public ApiResult<PageData<CardListResp>> queryCardSendList(@ModelAttribute ICCardQueryParam queryVo){
    ...
    public ApiResult sendCard(HttpServletRequest request, @RequestBody List<SendAndReadCardParam> param){
        ApiResult result=new ApiResult();
        String sessionId = request.getSession().getId();

        icCardWrapper.sendCard(param,sessionId);
    ...
    public ApiResult readCard(HttpServletRequest request, @RequestBody List<SendAndReadCardParam> param){
        ApiResult result=new ApiResult();
        String sessionId = request.getSession().getId();

        icCardWrapper.readCard(param,sessionId);
        //userLogService.create(currentUser.getId(), Action.MODIFY.getActionId(), "充电桩IC卡", "", "","下发["+param.getCardIds().toString()+"]充电IC卡",JSON.toJSONString(param));
        result.setMessage("发送指令成功！");
        return result;
    }
    ...
    @ApiImplicitParam(name = "commNos", value = "设备通信编号集合", required = true)
    @PutMapping(value = "/clearCard")
    public ApiResult clearCard(HttpServletRequest request, @RequestBody List<String> commNos){
        ApiResult result=new ApiResult();
        String sessionId = request.getSession().getId();

        icCardWrapper.clearCard(commNos,sessionId);
        //userLogService.create(currentUser.getId(), Action.MODIFY.getActionId(), "充电桩IC卡", "", "","下发["+param.getCardIds().toString()+"]充电IC卡",JSON.toJSONString(param));
        result.setMessage("发送指令成功！");
        return result;
    }
    ...
    @ApiImplicitParam(name = "ICCardQueryParam", value = "ic卡充值记录列表查询参数", required = true)
    @GetMapping(value = "/payList")
    public ApiResult<PageData<CustomerPayCardInfo>> queryPayList(@ModelAttribute ICCardQueryParam queryVo){
        ApiResult result=new ApiResult();
        //查询数据库
    ...
    public ApiResult queryUseListByIC(@ModelAttribute ICCardQueryParam queryVo){
        ApiResult result=new ApiResult();
        //查询数据库
